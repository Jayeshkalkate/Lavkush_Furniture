...
def admin_finance_dashboard(request):
    ...
    if not request.user.is_staff:
        return redirect("homepage")  # safer redirect
...
# ===============================
# 📊 USER FINANCE DASHBOARD (READ ONLY)
# ===============================
...
def customer_finance_dashboard(request):
    ...
    total = (
        Payment.objects.filter(user=request.user, status="paid").aggregate(
            Sum("amount")
        )["amount__sum"]
        or 0
    )
    ...
    refunded = (
        Payment.objects.filter(user=request.user, status="refunded").aggregate(
            Sum("refund_amount")
        )["refund_amount__sum"]
        or 0
    )
...
def download_receipt_pdf(request, payment_id):
    ...
    elements.append(
        Paragraph(
            f"Date: {payment.created_at.strftime('%d %b %Y %H:%M')}", styles["Normal"]
        )
    )
